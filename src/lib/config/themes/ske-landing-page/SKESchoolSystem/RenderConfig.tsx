    ...
    return (
      <div className="flex flex-col-reverse gap-6 sm:flex-row sm:gap-[5.75rem]">
        <div className="rounded-xl transition-opacity duration-500 ease-in-out opacity-100 sm:rounded-3xl">
          <div className="bg-white relative h-[26.75rem] rounded-xl shadow-md overflow-hidden w-full transition-opacity duration-500 ease-in-out opacity-100 sm:h-[36.25rem] sm:w-[30.625rem] sm:rounded-3xl">
            <div className="bg-white px-2 pt-2 h-[24.75rem] w-full group">
              <div className="relative w-full h-[17.875rem] sm:h-[24.75rem] overflow-hidden rounded-t-lg sm:rounded-t-3xl">
                <img
                  src={`${imageUrl ? configs.API_URL + imageUrl : "https://randomuser.me/api/portraits/women/50.jpg"}`}
                  alt="banner"
        ...
        <div className="flex flex-col justify-center gap-6 sm:gap-12">
          ...
          <div className="grid grid-cols-2 sm:grid-rows-3 gap-4 sm:gap-x-[3.563rem] sm:gap-y-8">
            {items.map((button, index) => (
              <button
                key={index}
                className={`flex justify-start items-start group gap-1 sm:gap-4 text-left text-4.25 leading-5.25 font-medium p-3 pr-0 whitespace-nowrap sm:text-lg sm:font-semibold sm:leading-6 sm:pl-0 xl:pl-10 sm:py-5 rounded-lg sm:rounded-xl border border-white hover:bg-white hover:text-green ${selectedButton === index
