      ...
      <>
        ...
        >
          {slides.map(({ id, image, imageHover }, index) => (
            ...
            >
              <div
                className={`gridItem px-[10px] md:px-0 w-[474px] sm:rounded-[25px] rounded-[15px] max-sm:w-full max-sm:mt-[10px] sm:pb-[3px] pb-0`}
                onClick={() => handlePreview(imageHover)}
              >
                <img
                  src={image ? `${configs.API_URL}${image}` : DefaultImage}
                  alt={`Slide ${index}`}
                  className={`md:w-[474px] max-sm:w-full max-sm:h-[153px] hover:scale-110 duration-500 ease-in-out transform sm:rounded-[25px] rounded-[15px]`}
                />
              </div>
            </div>
          ))}
